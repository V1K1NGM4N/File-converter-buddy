...
export class XMLFeedParser {
  ...
  private extractImagesFromContent(itemContent: string): ProductImage[] {
    ...
    const mainImageMatch = itemContent.match(/<g:image_link[^>]*>([^<]+)<\/g:image_link>/i);
    if (mainImageMatch) {
    ...
    const additionalImageMatches = itemContent.match(/<g:additional_image_link[^>]*>([^<]+)<\/g:additional_image_link>/gi);
    if (additionalImageMatches) {
      additionalImageMatches.forEach((match, index) => {
        const urlMatch = match.match(/<g:additional_image_link[^>]*>([^<]+)<\/g:additional_image_link>/i);
        if (urlMatch) {
    ...
    // Extract regular image_link tags
    const imageLinkMatches = itemContent.match(/<image_link[^>]*>([^<]+)<\/image_link>/gi);
    if (imageLinkMatches) {
      imageLinkMatches.forEach((match, index) => {
        const urlMatch = match.match(/<image_link[^>]*>([^<]+)<\/image_link>/i);
        if (urlMatch) {
          const url = urlMatch[1].trim();
          if (this.isValidImageUrl(url)) {
            images.push({
...
export async function downloadImagesAsZip(images: { url: string; filename: string }[]): Promise<void> {
  try {
    ...
    // to create a proper ZIP file on the client side
